...
const SUPABASE_ANON_KEY = 'sb_publishable_EZ9nHMJSwmFfP9c7W36fWw_pZVPIble';
const SUPABASE_TABLE = 'rds_kv';
const AUTH_SESSION_KEY = 'rds_auth_session_v1';
const SUPABASE_SYNC_INTERVAL_MS = 15000;
const SUPABASE_BOOT_RELOAD_KEY = 'rds_supabase_boot_reload_done_v1';
let __supabaseSyncGuard = false;
let __supabaseSyncTimer = null;
let __supabasePullInProgress = false;
let __supabaseReady = false;
let __supabaseQueueTimer = null;
...
async function initAuth(){
  ...
  let initialChangedKeys = [];
  try{ initialChangedKeys = await supabaseLoadAll(); }catch(e){}
  __supabaseReady = true;

  try{
    if(Array.isArray(initialChangedKeys) && initialChangedKeys.length){
      const alreadyReloaded = sessionStorage.getItem(SUPABASE_BOOT_RELOAD_KEY) === '1';
      if(!alreadyReloaded){
        sessionStorage.setItem(SUPABASE_BOOT_RELOAD_KEY, '1');
        location.reload();
        return;
      }
    } else {
      sessionStorage.removeItem(SUPABASE_BOOT_RELOAD_KEY);
    }
  }catch(e){ /* ignore */ }
...
async function getValidBearerToken(){
  ...
  if(isAuthExpired(session)){
    try{
      ...
      return refreshed?.access_token || SUPABASE_ANON_KEY;
    }catch(e){
      clearAuthSession();
...
function stopSupabaseAutoSync(){
  if(__supabaseSyncTimer){
    clearInterval(__supabaseSyncTimer);
    __supabaseSyncTimer = null;
  }
}

function stopSupabaseWriteQueue(){
  if(__supabaseQueueTimer){
    clearInterval(__supabaseQueueTimer);
    __supabaseQueueTimer = null;
  }
}

function ensureSupabaseWriteQueue(){
  if(__supabaseQueueTimer || isLoginPage()) return;
  __supabaseQueueTimer = setInterval(()=>{ flushSupabaseWriteQueue(); }, 4000);
  flushSupabaseWriteQueue();
}

function queueSupabaseUpsert(key, value){
  if(!key) return;
  __supabaseWriteQueue.set(key, { op: 'upsert', value, ts: Date.now() });
  ensureSupabaseWriteQueue();
}

function queueSupabaseDelete(key){
  if(!key) return;
  __supabaseWriteQueue.set(key, { op: 'delete', ts: Date.now() });
  ensureSupabaseWriteQueue();
}

async function flushSupabaseWriteQueue(){
  if(__supabaseQueueInProgress || !__supabaseReady || !__supabaseWriteQueue.size) return;
  __supabaseQueueInProgress = true;
  try{
    for(const [key, item] of Array.from(__supabaseWriteQueue.entries())){
      let ok = false;
      if(item.op === 'delete') ok = await supabaseDelete(key);
      else ok = await supabaseUpsert(key, item.value);
      if(ok) __supabaseWriteQueue.delete(key);
    }
  }finally{
    __supabaseQueueInProgress = false;
    if(!__supabaseWriteQueue.size) stopSupabaseWriteQueue();
  }
}

function startSupabaseAutoSync(){
  if(__supabaseSyncTimer || isLoginPage()) return;

  const pullNow = async ()=>{
    if(__supabasePullInProgress) return;
    __supabasePullInProgress = true;
    try{
      const changedKeys = await supabaseLoadAll();
      if(Array.isArray(changedKeys) && changedKeys.length){
        if(!document.hidden){
          location.reload();
        }
      }
    }finally{
      __supabasePullInProgress = false;
    }
  };
...
async function supabaseRequest(path, options = {}){
  ...
  let token = await getValidBearerToken();
  ...
  if(res.status === 401){
    ...
    if(session?.refresh_token){
      try{
        const refreshed = await refreshAuthSession(session);
        token = refreshed?.access_token || SUPABASE_ANON_KEY;
...
async function supabaseLoadAll(){
  ...
  try{
    ...
    rows.forEach(r=>{
      if(!r || !r.key) return;
      if(!String(r.key).startsWith('rds_') || r.key === AUTH_SESSION_KEY) return;
      if(__supabaseWriteQueue.has(r.key)) return;
